...
vector<Ciphertext> new_expand_query(const EncryptionParameters& parms, const std::vector<Ciphertext>& cts, uint32_t total_items,
    ...
    /*
        Briefly analyze the principle of new_expand_query.
        Consider two dimensions, each of length d1 and d2.
        Essentially, we need to expand a single ciphertext into vectors of length d1 and d2 in each dimension.
        In SealPIR, the query index of each dimension requires at least one ciphertext to store.
        In fact, there is a lot of waste in this.
        Considering d1=4, d2=4, here only 8 coefficients are needed to represent the query index of each dimension,
        and one PT has N coefficients, so when dim_sum <= N, one PT can be used to represent all dimensions query index.
...
vector<Ciphertext> multiply_mulpir( const EncryptionParameters& parms,  
    ...
    for (size_t i = 0; i < this_dimension; ++i) {
        ...
        if (remaining_dimensions.empty()) {
            ...
            // enter recursion
            vector<Ciphertext> lower_result = multiply_mulpir(parms, relin_keys, database, database_it, selection_vector, selection_vector_it + this_dimension, remaining_dimensions, depth + 1);
