 ...
 * @date 2022/11/15
 */
public class HeavyGuardianStreamCounter<T> implements StreamCounter<T> {
    /**
     * b = 1.08
     */
     ...
     */
    private final ArrayList<Map<T, Integer>> heavyPart;
    /**
    ...
    public HeavyGuardianStreamCounter(int w, int lambdaH, int lambdaL) {
        this(w, lambdaH, lambdaL, 0);
    }

    public HeavyGuardianStreamCounter(int w, int lambdaH, int lambdaL, int primeIndex) {
        ...
        heavyPart = IntStream.range(0, w)
            .mapToObj(bucketIndex -> new HashMap<T, Integer>(lambdaH))
        ...
        lightPart = IntStream.range(0, w)
            .mapToObj(bucketIndex -> new HashMap<T, Integer>(lambdaL))
            .collect(Collectors.toCollection(ArrayList::new));
        // init bob hash
    ...
    public boolean insert(T item) {
        ...
        Map<T, Integer> heavyPartBucket = heavyPart.get(bucketIndex);
        ...
        List<Map.Entry<T, Integer>> heavyPartList = new ArrayList<>(heavyPartBucket.entrySet());
        heavyPartList.sort(Comparator.comparingInt(Map.Entry::getValue));
        Map.Entry<T, Integer> weakestHeavyPartCell = heavyPartList.get(0);
        T weakestHeavyPartItem = weakestHeavyPartCell.getKey();
        ...
        Map<T, Integer> lightPartBucket = lightPart.get(bucketIndex);
