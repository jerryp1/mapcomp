...
public class FieldElementTest {
    ...
    public void testAMulAVsASquaredConstant() {
        final FieldElement a = FieldElement.decode(A_BYTES);
        final FieldElement asq = FieldElement.decode(ASQ_BYTES);
        assertThat(a.mul(a), is(asq));
    }
    ...
    public void testAInvertVsInverseOfAConstant() {
        final FieldElement a = FieldElement.decode(A_BYTES);
        final FieldElement ainv = FieldElement.decode(AINV_BYTES);
        final FieldElement shouldBeInverse = a.inv();
        assertThat(shouldBeInverse, is(ainv));
        assertThat(a.mul(shouldBeInverse), is(FieldElement.ONE));
    }
    ...
    public void sqrtRatioM1Behavior() {
        ...
        assertThat(sqrt.wasSquare, is(0));
        assertThat(sqrt.result.sqr(), is(two.mul(i)));
        assertThat(sqrt.result.isNegative(), is(0));

        // 4/1 is square, so we expect (1, sqrt(4))
        sqrt = FieldElement.sqrtRatioM1(four, one);
        assertThat(sqrt.wasSquare, is(1));
        assertThat(sqrt.result.sqr(), is(four));
        assertThat(sqrt.result.isNegative(), is(0));

        // 1/4 is square, so we expect (1, 1/sqrt(4))
        sqrt = FieldElement.sqrtRatioM1(one, four);
        assertThat(sqrt.wasSquare, is(1));
        assertThat(sqrt.result.sqr().mul(four), is(one));
        assertThat(sqrt.result.isNegative(), is(0));
    }

    @Test
    public void testAP58VsAP58Constant() {
        FieldElement a = FieldElement.decode(A_BYTES);
        FieldElement ap58 = FieldElement.decode(AP58_BYTES);
        assertThat(a.powPm5d8(), is(ap58));
    }
    ...
    public void fromByteArrayHighbitIsIgnored() {
        ...
        FieldElement withHighbitSet = FieldElement.decode(B_BYTES);
        FieldElement withoutHighbitSet = FieldElement.decode(cleared_bytes);
    ...
    public void encodingIsCanonical() {
        ...
        FieldElement one = FieldElement.decode(oneEncodedWronglyBytes);
    ...
    public void encodeAndDecodeOnZero() {
        ...
        final FieldElement a = FieldElement.decode(zero);

        assertThat(a, is(FieldElement.ZERO));
        assertThat(a.encode(), is(zero));
    }
    ...
    public void ctSelectReturnsCorrectResult() {
        ...
        assertThat(a.cmov(b, 0), is(a));
        assertThat(a.cmov(b, 1), is(b));
        assertThat(b.cmov(a, 0), is(b));
        assertThat(b.cmov(a, 1), is(a));
    }
}
