...
public class RistrettoElementTest {
    ...
    public void invalidEncodingsAreRejected() {
        for (int i = 0; i < INVALID_ENCODINGS.length; i++) {
            CompressedRistretto s = new CompressedRistretto(Hex.decode(INVALID_ENCODINGS[i]));
            try {
                s.decompress();
    ...
    public void fromUniformBytes() {
        ...
        for (int i = 0; i < FROM_UNIFORM_BYTES_INPUTS.length; i++) {
            RistrettoElement P = RistrettoElement.fromUniformBytes(Hex.decode(FROM_UNIFORM_BYTES_INPUTS[i]));
            assertThat(P.compress(), is(new CompressedRistretto(Hex.decode(FROM_UNIFORM_BYTES_OUTPUTS[i]))));
        }
    }
}
