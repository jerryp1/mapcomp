...
abstract class AbstractZl implements Zl {
    ...
    public BigInteger createRandom(byte[] seed) {
        ...
        return module(BigIntegerUtils.byteArrayToNonNegBigInteger(elementByteArray));
    }
    ...
    public BigInteger createNonZeroRandom(byte[] seed) {
        byte[] key = kdf.deriveKey(seed);
        byte[] elementByteArray = prg.extendToBytes(key);
        BigInteger random = module(BigIntegerUtils.byteArrayToNonNegBigInteger(elementByteArray));
        while (random.equals(BigInteger.ZERO)) {
            // 如果恰巧为0，则迭代种子
            key = kdf.deriveKey(key);
            elementByteArray = prg.extendToBytes(key);
            random = module(BigIntegerUtils.byteArrayToNonNegBigInteger(elementByteArray));
        }
        return random;
    }
