...
class ProjectiveNielsPoint {
    ...
    public ProjectiveNielsPoint ctSelect(ProjectiveNielsPoint that, int b) {
        return new ProjectiveNielsPoint(this.YPlusX.cmov(that.YPlusX, b), this.YMinusX.cmov(that.YMinusX, b),
                this.Z.cmov(that.Z, b), this.T2D.cmov(that.T2D, b));
    }
    ...
    static class LookupTable {
        ...
        ProjectiveNielsPoint select(final int x) {
            ...
            for (int i = 1; i < 9; i++) {
                t = t.ctSelect(this.table[i - 1], CafeConstantTimeUtils.equal(xabs, i));
            }
