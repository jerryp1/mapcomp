...
public final class Poly {
    ...
    public static short[][] polyVectorFromBytes(byte[] polyA) {
        int paramsK = polyA.length/KyberParams.paramsPolyBytes;
    ...
    public static short[] decompressPoly(byte[] a, int paramsK) {
        ...
        switch (paramsK) {
            ...
            case 3:
                for (int i = 0; i < KyberParams.paramsN / 2; i++) {
                    r[2 * i] = (short) ((((int) (a[i] & 15) * KyberParams.paramsQ) + 8) >> 4);
                    r[2 * i + 1] = (short) (((((int) (a[i] & 0xFF) >> 4) * KyberParams.paramsQ) + 8) >> 4);
                }
                break;
            default:
                int aa = 0;
