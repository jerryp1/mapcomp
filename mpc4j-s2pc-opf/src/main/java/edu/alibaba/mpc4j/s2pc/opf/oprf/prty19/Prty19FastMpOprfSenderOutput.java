...
//                .collect(Collectors.toList());
    ...
    public byte[] getPrf(int index, byte[] input) {
        ...
        byte[] px = okvsList.get(binIndex).decode(Arrays.copyOfRange(storage,binIndex * (storage.length / binNum), (binIndex + 1) * (storage.length / binNum)), ByteBuffer.wrap(h1.digestToBytes(ByteBuffer.allocate(prfByteLength + 1).put(h1.digestToBytes(input)).put(Integer.valueOf(index).byteValue()).array())));
