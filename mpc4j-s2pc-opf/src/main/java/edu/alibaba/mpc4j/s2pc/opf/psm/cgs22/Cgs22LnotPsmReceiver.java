...
public class Cgs22LnotPsmReceiver extends AbstractPsmReceiver {
    ...
    public void init(int maxL, int d, int maxNum) throws MpcAbortException {
        ...
        int maxQ = maxByteL * 2;
        bcReceiver.init(maxNum, maxNum * (maxQ - 1) * d);
    ...
    private SquareZ2Vector combine(BitVector[][] eqArrays, int q) throws MpcAbortException {
        ...
        SquareZ2Vector[] eq1s = IntStream.range(0, d)
            .mapToObj(index -> (SquareZ2Vector) bcReceiver.createOnes(num))
        ...
        for (int i = 0; i < d; i++) {
            ...
            for (int j = 0; j < q; j++) {
                ...
                eq1s[i] = bcReceiver.and(eq1s[i], eqArrays1[i][j]);
                eqArrays1[i][j] = null;
            }
        }
