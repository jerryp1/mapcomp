...
public class Gmr21OsnSender extends AbstractOsnSender {
    ...
    public OsnPartyOutput osn(Vector<byte[]> inputVector, int byteLength) throws MpcAbortException {
        ...
        sendOtherPartyPayload(PtoStep.SENDER_SEND_INPUT_CORRECTIONS.ordinal(), Arrays.stream(sendMsg).collect(Collectors.toList()));
        OsnPartyOutput senderOutput = new OsnPartyOutput(byteLength, Arrays.stream(senderShareVector).collect(Collectors.toCollection(Vector::new)));
        switchWireMask0s = null;
        switchWireMask1s = null;
    ...
    private List<byte[]> generateSwitchCorrectionPayload() {
        senderShareVector = BytesUtils.randomByteArrayVector(n, byteLength, secureRandom);
