...
public class Decryptor {
    ...
    /**
     * Decrypts a BFV ciphertext and stores the result in the destination.
     *
     * @param encrypted   the ciphertext to decrypt.
     * @param destination the plaintext to overwrite with the decrypted ciphertext.
     */
    ...
    /**
     * Computes c_0 + c_1 *s + ... + c_{count-1} * s^{count-1} mod q and stores result in destination in RNS form.
     *
     * @param encrypted   the ciphertext.
     * @param destination the array to overwrite with the result of dot product.
     */
    private void dotProductCtSkArray(Ciphertext encrypted, long[] destination) {
        ...
        if (encryptedSize == 2) {
            ...
            for (int i = 0; i < (encryptedSize - 1); i++) {
                ...
                // encryptedCopy 的 k 是 coeff_modulus_size,
                // secretKeyArray 的 k 是 key_coeff_modulus_size, modulus switch之前的modulus size
                int rnsIterStartIndex1 = i * coeffCount * coeffModulusSize;
                int rnsIterStartIndex2 = i * coeffCount * keyCoeffModulusSize;
     ...
     * The invariant noise budget measures the amount of room there is for the noise to grow
     * while ensuring correct decryptions. This function works only with the BFV scheme.
     *
     * @param encrypted the ciphertext.
