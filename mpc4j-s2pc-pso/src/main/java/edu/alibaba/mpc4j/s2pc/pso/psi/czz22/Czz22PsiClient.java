...
public class Czz22PsiClient <T> extends AbstractPsiClient<T> {
    /**
     * MqRpmt接收方
     */
    ...
    private Set<T> handleServerCipherPayload(List<byte[]> serverPayload){
        ...
        Set<ByteBuffer> serverIntersectionSet = clientVectorStream.mapToObj(index -> {
            if(clientVector[index]) {
                SecretKeySpec secretKeySpec = new SecretKeySpec(keyHash.digestToBytes(cotReceiverOutput.getRb(index)), Czz22PsiConfig.Cipher_ALGORITHM_NAME);
                Cipher decryptCipher;
                try{
                    decryptCipher = Cipher.getInstance(Czz22PsiConfig.Cipher_MODE_NAME);
