...
public abstract class AbstractSqOprfPsiServer<T> extends AbstractPsiServer<T> {
    ...
    public void setup(int maxServerElementSize, int maxClientElementSize, Set<T> serverElementSet) throws MpcAbortException {
        setInitInput(maxServerElementSize, maxClientElementSize);
        logPhaseInfo(PtoState.INIT_BEGIN);
        ...
        stopWatch.start();
        setPtoInput(serverElementSet, maxClientElementSize);
        int peqtByteLength = PsiUtils.getSemiHonestPeqtByteLength(maxServerElementSize, maxClientElementSize);
        Hash peqtHash = HashFactory.createInstance(envType, peqtByteLength);
        Stream<T> serverElementStream = serverElementArrayList.stream();
        serverElementStream = parallel ? serverElementStream.parallel() : serverElementStream;
        List<byte[]> serverPrfs = serverElementStream
        ...
        stopWatch.reset();
        logStepInfo(PtoState.INIT_STEP, 2, 2, sendTime);

        logPhaseInfo(PtoState.INIT_END);
    }

    public void psiOnline(int clientElementSize) throws MpcAbortException {
        logPhaseInfo(PtoState.PTO_BEGIN);
        ...
        long setupTime = stopWatch.getTime(TimeUnit.MILLISECONDS);
        stopWatch.reset();
        logStepInfo(PtoState.PTO_STEP, 1, 2, setupTime, "Server inits tools");
        ...
        long oprfTime = stopWatch.getTime(TimeUnit.MILLISECONDS);
        stopWatch.reset();
        logStepInfo(PtoState.PTO_STEP, 2, 2, oprfTime, "Server runs sq-OPRF");
