 ...
 * @date 2023/7/21
 */
public class Sj23PmtUcpsiClient<T> extends AbstractUcpsiClient<T> {
    /**
    ...
    public Sj23PmtUcpsiClient(Rpc clientRpc, Party serverParty, Sj23PmtUcpsiConfig config) {
        ...
        pesmSender = PesmFactory.createSender(clientRpc, serverParty, config.getPesmConfig());
        addSubPtos(pesmSender);
        pesmReceiver = PesmFactory.createReceiver(clientRpc, serverParty, config.getPesmConfig());
    ...
    public UcpsiClientOutput<T> psi(Set<T> clientElementSet) throws MpcAbortException {
        ...
        if (isHeReceiver) {
            pemtOutput = pesmReceiver.pesm(params.l, alpha, mask);
        } else {
            pemtOutput = pesmSender.pesm(params.l, response);
        }
    ...
    private List<byte[]> generateQuery() {
        ...
        return queryStream
            .map(i -> Sj23PmtUcpsiNativeUtils.generateQuery(params.encryptionParams, clientPublicKey, clientSecretKey, i))
    ...
    private byte[][][] decodeResponse(List<byte[]> responsePayload) throws MpcAbortException {
        ...
        List<long[]> coeffs = responseStream
            .map(i -> Sj23PmtUcpsiNativeUtils.decodeReply(params.encryptionParams, clientSecretKey, i))
            .collect(Collectors.toCollection(ArrayList::new));
