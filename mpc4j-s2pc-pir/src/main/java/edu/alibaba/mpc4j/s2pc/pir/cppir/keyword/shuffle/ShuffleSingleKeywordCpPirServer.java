...
import edu.alibaba.mpc4j.s2pc.pir.cppir.keyword.shuffle.ShuffleSingleKeywordCpPirDesc.PtoStep;
...
/**
 * Shuffle keyword client-specific preprocessing PIR server.
 *
 * @author Liqiang Peng
 * @date 2023/9/20
 */
public class ShuffleSingleKeywordCpPirServer<T> extends AbstractSingleKeywordCpPirServer<T> {
    /**
     * sq-OPRF sender
     */
    ...
    public ShuffleSingleKeywordCpPirServer(Rpc serverRpc, Party clientParty, ShuffleSingleKeywordCpPirConfig config) {
        super(ShuffleSingleKeywordCpPirDesc.getInstance(), serverRpc, clientParty, config);
    ...
    public void init(Map<T, byte[]> keyValueMap, int labelBitLength) throws MpcAbortException {
        ...
        stopWatch.start();
        rowNum = ShuffleSingleKeywordCpPirUtils.getRowNum(n);
        columnNum = ShuffleSingleKeywordCpPirUtils.getColumnNum(n);
    ...
    private SingleKeywordCpPirServerOutput respondActualQuery(List<byte[]> queryRequestPayload) throws MpcAbortException {
        ...
        if (finalDatabase.containsKey(keyByteBuffer)) {
            ...
            return SingleKeywordCpPirServerOutput.IN;
        } else {
            logPhaseInfo(PtoState.PTO_BEGIN);
            ...
            logPhaseInfo(PtoState.PTO_END);
            return SingleKeywordCpPirServerOutput.OUT;
        }
    }
}