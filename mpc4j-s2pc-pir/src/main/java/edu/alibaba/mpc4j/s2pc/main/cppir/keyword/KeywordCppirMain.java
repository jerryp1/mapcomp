...
import edu.alibaba.mpc4j.s2pc.pir.cppir.keyword.SingleKeywordCpPirClient;
...
import edu.alibaba.mpc4j.s2pc.pir.cppir.keyword.SingleKeywordCpPirServer;
 ...
 * @date 2023/9/27
 */
public class KeywordCppirMain {
    private static final Logger LOGGER = LoggerFactory.getLogger(KeywordCppirMain.class);
    /**
     * task name
     */
    public static final String TASK_NAME = "KEYWORD_CPPIR_TASK";
    /**
     * protocol name
     */
    public static final String PTO_TYPE_NAME = "KEYWORD_CPPIR_PTO";
    /**
    ...
    public KeywordCppirMain(Properties properties) {
    ...
    private void runServer(Rpc serverRpc, Party clientParty) throws Exception {
        ...
        SingleKeywordCpPirConfig config = KeywordCppirConfigUtils.createKeywordCpPirConfig(properties);
    ...
    private void warmupServer(Rpc serverRpc, Party clientParty, SingleKeywordCpPirConfig config, int taskId)
        throws Exception {
        ...
        SingleKeywordCpPirServer<String> server = SingleKeywordCpPirFactory.createServer(serverRpc, clientParty, config);
    ...
    private void runServer(Rpc serverRpc, Party clientParty, SingleKeywordCpPirConfig config, int taskId,
                           boolean parallel, byte[][] elementArray, int elementBitLength, int queryNum,
        ...
        Map<String, byte[]> keywordValueMap = IntStream.range(0, elementArray.length)
            .boxed()
            .collect(Collectors.toMap(
                String::valueOf,
                i -> elementArray[i],
        ...
        stopWatch.start();
        server.init(keywordValueMap, elementBitLength);
        stopWatch.stop();
    ...
    private void warmupClient(Rpc clientRpc, Party serverParty, SingleKeywordCpPirConfig config, int taskId)
        throws Exception {
        LOGGER.info(
            "{}: serverSetSize = {}, elementBitLength = {}, queryNumber = {}, parallel = {}",
        ...
        SingleKeywordCpPirClient<String> client = SingleKeywordCpPirFactory.createClient(clientRpc, serverParty, config);
    ...
    private void runClient(Rpc clientRpc, Party serverParty, SingleKeywordCpPirConfig config, int taskId,
