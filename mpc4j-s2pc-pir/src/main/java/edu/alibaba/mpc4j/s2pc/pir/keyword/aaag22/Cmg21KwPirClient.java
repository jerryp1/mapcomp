...
import edu.alibaba.mpc4j.s2pc.pir.keyword.aaag22.Cmg21KwPirPtoDesc.PtoStep;
...
public class Cmg21KwPirClient<T> extends AbstractKwPirClient<T> {
    ...
    public Cmg21KwPirClient(Rpc clientRpc, Party serverParty, Aaag22KwPirConfig config) {
    ...
    public void init(int maxRetrievalSize, int labelByteLength) throws MpcAbortException {
        ...
        if (maxRetrievalSize > 1) {
            params = Aaag22KwPirParams.SERVER_1M_CLIENT_MAX_4096;
        } else {
            params = Aaag22KwPirParams.SERVER_1M_CLIENT_MAX_1;
        }
