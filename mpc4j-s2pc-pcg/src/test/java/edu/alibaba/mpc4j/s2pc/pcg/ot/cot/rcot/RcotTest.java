...
public class RcotTest {
    private static final Logger LOGGER = LoggerFactory.getLogger(RcotTest.class);
    /**
     * 随机状态
     */
    ...
    public static Collection<Object[]> configurations() {
        ...
        configurationParams.add(new Object[] {
            RcotType.KOS15.name(), new Kos15RcotConfig.Builder().build(),
        });
        // ALSZ13
        configurationParams.add(new Object[] {
            RcotType.ALSZ13.name(), new Alsz13RcotConfig.Builder().build(),
        });
        // IKNP03
        configurationParams.add(new Object[] {
            RcotType.IKNP03.name(), new Iknp03RcotConfig.Builder().build(),
        });
     ...
     * 协议类型
     */
    private final RcotConfig config;

    public RcotTest(String name, RcotConfig config) {
    ...
    public void testPtoType() {
        RcotSender sender = RcotFactory.createSender(senderRpc, receiverRpc.ownParty(), config);
        RcotReceiver receiver = RcotFactory.createReceiver(receiverRpc, senderRpc.ownParty(), config);
    ...
    private void testPto(RcotSender sender, RcotReceiver receiver, int num) {
        ...
        try {
            ...
            RcotSenderThread senderThread = new RcotSenderThread(sender, delta, num);
            RcotReceiverThread receiverThread = new RcotReceiverThread(receiver, choices);
    ...
    public void testResetDelta() {
        ...
        try {
            ...
            // 第一次执行
            RcotSenderThread senderThread = new RcotSenderThread(sender, delta, DEFAULT_NUM);
            RcotReceiverThread receiverThread = new RcotReceiverThread(receiver, choices);
            StopWatch stopWatch = new StopWatch();
            stopWatch.start();
            senderThread.start();
            ...
            SECURE_RANDOM.nextBytes(delta);
            IntStream.range(0, DEFAULT_NUM).forEach(index -> choices[index] = SECURE_RANDOM.nextBoolean());
            senderThread = new RcotSenderThread(sender, delta, DEFAULT_NUM);
            receiverThread = new RcotReceiverThread(receiver, choices);
            stopWatch.start();
            senderThread.start();
            receiverThread.start();
            senderThread.join();
