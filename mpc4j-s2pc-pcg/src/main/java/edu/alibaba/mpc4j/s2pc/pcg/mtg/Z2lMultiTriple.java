package edu.alibaba.mpc4j.s2pc.pcg.mtg;
...
public class Z2lMultiTriple {
     ...
     * @param cs 随机分量c。
     */
    public static Z2lMultiTriple create(int l, int n, byte[][] as, byte[][] bs, byte[][] cs) {
        ...
        Z2lMultiTriple z2lMultiTriple = new Z2lMultiTriple();
        z2lMultiTriple.l = l;
        z2lMultiTriple.byteL = byteL;
        z2lMultiTriple.n = n;
        z2lMultiTriple.as = Arrays.stream(as)
            .peek(a -> {
                assert a.length == byteL && BytesUtils.isReduceByteArray(a, l);
            })
            .map(BytesUtils::clone)
            .toArray(byte[][]::new);
        z2lMultiTriple.bs = Arrays.stream(bs)
            .peek(b -> {
                assert b.length == byteL && BytesUtils.isReduceByteArray(b, l);
            })
            .map(BytesUtils::clone)
            .toArray(byte[][]::new);
        z2lMultiTriple.cs = Arrays.stream(cs)
            .peek(c -> {
     ...
     */
    public static Z2lMultiTriple createEmpty(int l) {
        // 验证随机分量的长度
        assert l > 0 : "l must be greater than 0";
        int byteL = CommonUtils.getByteLength(l);

        Z2lMultiTriple emptyTriple = new Z2lMultiTriple();
        emptyTriple.l = l;
     ...
     * @param length 指定切分长度。
     */
    public Z2lMultiTriple split(int length) {
        ...
        return Z2lMultiTriple.create(l, length, aSubs, bSubs, cSubs);
    }
     ...
     * @param that 另一个乘法三元组。
     */
    public void merge(Z2lMultiTriple that) {
