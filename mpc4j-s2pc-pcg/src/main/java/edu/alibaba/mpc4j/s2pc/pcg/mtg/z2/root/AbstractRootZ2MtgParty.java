package edu.alibaba.mpc4j.s2pc.pcg.mtg.z2.root;

import edu.alibaba.mpc4j.common.rpc.Party;
import edu.alibaba.mpc4j.common.rpc.Rpc;
import edu.alibaba.mpc4j.common.rpc.desc.PtoDesc;
import edu.alibaba.mpc4j.common.rpc.pto.AbstractSecureTwoPartyPto;
import edu.alibaba.mpc4j.common.tool.utils.CommonUtils;

/**
 * 根布尔三元组生成协议。
 *
 * @author Weiran Liu
 * @date 2022/7/13
 */
public abstract class AbstractRootZ2MtgParty extends AbstractSecureTwoPartyPto implements RootZ2MtgParty {
    /**
     * 配置项
     */
    protected final RootZ2MtgConfig config;
    /**
     * 最大数量
     */
    protected int maxNum;
    /**
     * 数量
     */
    protected int num;
    /**
     * 字节数量
     */
    protected int byteNum;

    public AbstractRootZ2MtgParty(PtoDesc ptoDesc, Rpc ownRpc, Party otherParty, RootZ2MtgConfig config) {
        super(ptoDesc, ownRpc, otherParty, config);
        this.config = config;
    }

    @Override
    public RootZ2MtgFactory.RbtgType getPtoType() {
        return config.getPtoType();
    }

    protected void setInitInput(int maxNum) {
        assert maxNum > 0 && maxNum <= config.maxAllowNum()
            : "maxNum must be in range (0, " + config.maxAllowNum() + "]: " + maxNum;
        this.maxNum = maxNum;
        initialized = false;
    }

    protected void setPtoInput(int num) {
        if (!initialized) {
            throw new IllegalStateException("Need init...");
        }
        assert num > 0 && num <= maxNum : "num must be in range (0, " + maxNum + "]: " + num;
        this.num = num;
        byteNum = CommonUtils.getByteLength(num);
        extraInfo++;
    }
}
