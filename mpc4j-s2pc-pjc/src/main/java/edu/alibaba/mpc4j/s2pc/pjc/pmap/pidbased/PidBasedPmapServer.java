...
public class PidBasedPmapServer<T> extends AbstractPmapServer<T> {
    ...
    public PidBasedPmapServer(Rpc serverRpc, Party clientParty, PidBasedPmapConfig config) {
        super(PidBasedPmapPtoDesc.getInstance(), serverRpc, clientParty, config);
        pidServer = PidFactory.createServer(serverRpc, clientParty, config.getPidConfig());
        peqtReceiver = PeqtFactory.createReceiver(serverRpc, clientParty, config.getPeqtConfig());
        addMultipleSubPtos(pidServer, peqtReceiver);
    }
    ...
    public void init(int maxServerElementSize, int maxClientElementSize) throws MpcAbortException {
        ...
        peqtReceiver.init(peqtByteL<<3, maxServerElementSize + maxClientElementSize);
    ...
    public PmapPartyOutput<T> map(List<T> serverElementList, int clientElementSize) throws MpcAbortException {
        ...
        rpc.send(DataPacket.fromByteArrayList(hashKeyHeader, Collections.singletonList(hashKeys)));
        ...
        SquareZ2Vector z1 = peqtReceiver.peqt(peqtByteL<<3, target);
