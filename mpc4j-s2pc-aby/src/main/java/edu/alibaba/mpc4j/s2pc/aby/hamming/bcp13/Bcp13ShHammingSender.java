...
public class Bcp13ShHammingSender extends AbstractHammingParty {
    ...
    @Override
    public void sendHammingDistance(BcSquareVector x0) throws MpcAbortException {
        setPtoInput(x0);
    ...
    @Override
    public int receiveHammingDistance(BcSquareVector x0) throws MpcAbortException {
        setPtoInput(x0);
        info("{}{} Send. begin", ptoBeginLogPrefix, getPtoDesc().getPtoName());
    ...
    private int executeOtSteps(BcSquareVector x0) throws MpcAbortException {
        ...
        List<byte[]> senderMessagePayload = IntStream.range(0, bitNum)
            .mapToObj(index -> {
                byte[] key0 = Arrays.copyOf(crhf.hash(cotSenderOutput.getR0(index)), messageByteLength);
                byte[] key1 = Arrays.copyOf(crhf.hash(cotSenderOutput.getR1(index)), messageByteLength);
